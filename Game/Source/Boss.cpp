...
bool Boss::Awake()
{
	...
	pugi::xml_node parentConfig = config;

	// Initilize Idle Animation
...
bool Boss::Start()
{
	...
	enemyRange = config.attribute("eRange").as_int();
	followVelovity = config.attribute("followVel").as_float();
	patrolVelocity = config.attribute("patrolVel").as_float();
...
void Boss::moveToPoint(float dt)
{
	Enemy::Patrol();
...
bool Boss::LoadEntity(pugi::xml_node& load)
{
...
bool Boss::SaveEntity(pugi::xml_node& save)
{
